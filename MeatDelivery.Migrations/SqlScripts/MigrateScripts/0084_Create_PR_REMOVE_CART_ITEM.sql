-- Migration Script: 0084_Create_PR_REMOVE_CART_ITEM.sql
-- Description: Create stored procedure for removing a selected item from the customer active cart.

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.PR_REMOVE_CART_ITEM') AND type in (N'P', N'PC'))
    DROP PROCEDURE dbo.PR_REMOVE_CART_ITEM;
GO

CREATE PROCEDURE dbo.PR_REMOVE_CART_ITEM
