...
BEGIN
    ...
    -- Find Active Cart and Attached Coupon
    SELECT TOP 1 
        @CART_ID = CART_ID,
    ...
    BEGIN TRY
        ...
        (
            ...
            @SUBTOTAL, 0.00, @APPLIED_COUPON_ID, ISNULL(@COUPON_DISCOUNT, 0.00),
        ...
        IF @APPLIED_COUPON_ID IS NOT NULL
        BEGIN
            INSERT INTO dbo.COUPON_USAGES
            (
                CUSTOMER_USER_ID, COUPON_ID, ORDER_ID, USAGE_STATUS, USED_AT
            )
            VALUES
            (
                @CUSTOMER_USER_ID, @APPLIED_COUPON_ID, @ORDER_ID, 'USED', SYSUTCDATETIME()
            );

            UPDATE dbo.COUPONS
            SET UPDATED_AT = SYSUTCDATETIME()
            WHERE COUPON_ID = @APPLIED_COUPON_ID;
        END;

        -- 4. Copy Items Set-Based using MERGE from TVP @ITEMS
        ...
        -- 5. Copy Customizations Set-Based from TVP @CUSTOMIZATIONS
