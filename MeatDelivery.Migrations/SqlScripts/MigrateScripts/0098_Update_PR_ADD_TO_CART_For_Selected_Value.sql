-- Migration Script: 0098_Update_PR_ADD_TO_CART_For_Selected_Value.sql
-- Description: Update PR_ADD_TO_CART and PR_UPDATE_CART_ITEM_CUSTOMIZATION to support dbo.TT_CUSTOMIZATION_SELECTIONS with SELECTED_VALUE.
...
BEGIN
    ...
    IF @CART_ITEM_ID IS NULL OR @CART_ITEM_ID <= 0
    BEGIN
        RAISERROR(''Valid CartItemId is required.'', 16, 1);
        RETURN;
    END;

    -- Validate Cart Item belongs to Active Customer Cart
    IF NOT EXISTS (
        SELECT 1
    ...
    BEGIN
        RAISERROR(''Cart item not found or does not belong to customer active cart.'', 16, 1);
        RETURN;
    END;
    ...
    BEGIN TRY
        DECLARE @CART_DOC_NO VARCHAR(50) = NULL;

        SELECT @CART_DOC_NO = c.DOC_NO
        FROM dbo.CARTS c
        INNER JOIN dbo.CART_ITEMS ci ON ci.CART_ID = c.CART_ID
        WHERE ci.CART_ITEM_ID = @CART_ITEM_ID;
        ...
        INSERT INTO dbo.CART_ITEM_CUSTOMIZATIONS (CART_ITEM_ID, DOC_NO, CUSTOMIZATION_OPTION_ID, SELECTED_VALUE, CREATED_AT)
        SELECT DISTINCT @CART_ITEM_ID, @CART_DOC_NO, OPTION_ID, SELECTED_VALUE, SYSUTCDATETIME()
        FROM @OPTION_SELECTIONS;
