-- Migration: 0052_Create_TVP_And_PR_MANAGE_PRODUCT_ATTRIBUTES.sql
-- Description: Creates User-Defined Table Type dbo.PRODUCT_ID_LIST and Stored Procedure dbo.PR_MANAGE_PRODUCT_ATTRIBUTES.

IF NOT EXISTS (SELECT 1 FROM sys.types WHERE name = 'PRODUCT_ID_LIST' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    CREATE TYPE dbo.PRODUCT_ID_LIST AS TABLE
    (
...
IF OBJECT_ID('dbo.PR_MANAGE_PRODUCT_ATTRIBUTES', 'P') IS NOT NULL
    DROP PROCEDURE dbo.PR_MANAGE_PRODUCT_ATTRIBUTES;
GO

IF OBJECT_ID('dbo.PR_BULK_UPDATE_PRODUCT_FLAGS', 'P') IS NOT NULL
    DROP PROCEDURE dbo.PR_BULK_UPDATE_PRODUCT_FLAGS;
GO

EXEC('
CREATE PROCEDURE dbo.PR_MANAGE_PRODUCT_ATTRIBUTES
(
    @MODE           VARCHAR(30),
