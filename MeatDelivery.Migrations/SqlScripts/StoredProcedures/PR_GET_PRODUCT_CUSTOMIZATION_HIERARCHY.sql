...
BEGIN
    ...
    WHERE p.PRODUCT_ID = @PRODUCT_ID AND t.IS_ACTIVE = 1 AND t.IS_DELETED = 0;
    ...
    WHERE p.PRODUCT_ID = @PRODUCT_ID AND g.IS_ACTIVE = 1 AND g.IS_DELETED = 0 AND m.IS_ACTIVE = 1 AND m.IS_DELETED = 0;
    ...
    INNER JOIN dbo.PRODUCTS p ON p.CUSTOMIZATION_TEMPLATE_ID = m.CUSTOMIZATION_TEMPLATE_ID
    WHERE p.PRODUCT_ID = @PRODUCT_ID AND o.IS_ACTIVE = 1 AND o.IS_DELETED = 0 AND m.IS_ACTIVE = 1 AND m.IS_DELETED = 0;
END;
GO
